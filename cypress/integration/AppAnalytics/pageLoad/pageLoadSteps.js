...
Then('there should be an NCIDataLayer PageLoad event with the name {string}', (eventName) => {
  cy.window().then((win) => {
 ...
 */
Then('the event has the following additional details for the page', (datatable) => {
//get the number of row without headers
const numberOfKeys = datatable.rawTable.slice().length - 1;
  cy.window().then((win) => {
