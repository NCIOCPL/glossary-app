...
Given('there is an audio player with the url {string} before the phonetic spelling', (a) => {
  cy.get('div.pronunciation__audio').as('player').should('exist');
...
Then('when audio has finished playing, the button returns to the audio speaker icon', () => {
  cy.get('div.pronunciation button').should('not.have.class', 'btnAudio playing');
  cy.document().then((doc) => {
...
Then('the pronunciation for {string} should play', (def) => {
  cy.get('div.pronunciation button').wait(100).should('have.class', 'btnAudio playing');
  cy.document().then((doc) => {
    let current = doc.querySelector('div.pronunciation__audio audio').currentTime
