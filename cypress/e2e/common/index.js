...
When('user is searching {string} for the term {string}', (searchMode, term) => {
	...
	cy.window().then((win) => {
		if (win.INT_TEST_APP_PARAMS) {
			const searchLang = win.INT_TEST_APP_PARAMS.language === 'en' ? queryType.SEARCH : queryType.SEARCH_SPANISH;
...
When('user tries to go to this URL, system should return the Expand {string} page for language {string}', () => {
	cy.window().then((win) => {
		if (win.INT_TEST_APP_PARAMS) {
			const { language } = win.INT_TEST_APP_PARAMS;
			cy.get(`p[data-testid='${testIds.NO_MATCHING_RESULTS}']`).should('have.text', i18n.noMatchingExpand[language]);
		}
	});
});
	...
	cy.document().then((doc) => {
		...
		for (const { term, href } of dataTable.hashes()) {
			terms.forEach((singleTerm) => {
				if (singleTerm.innerText === term) expect(singleTerm.href).to.eq(`${baseURL}${href}`);
			});
		}
	});
});
...
Then('search is executed', () => {
	...
	cy.document().then((doc) => {
		...
		if (selectedRadio === 'Starts with' || selectedRadio === 'Empieza con') searchMode = 'searchMode=Begins';
