...
const defaultProps = {
    ...
    renderItem: (item, isHighlighted) => (
        <div key={item.name} className={isHighlighted ? 'highlighted' : 'none'} role="option">
            {item.name}
        </div>
    ),
    renderMenu: children => (
    ...
    test("should set menu positions on render when menu is open", () => {
        ...
        render(
            <Autocomplete
                {...defaultProps}
                open
                renderMenu={onRenderMenuHandler}
            />
        );
        expect(onRenderMenuHandler).toHaveBeenCalledTimes(2);
        // Initial render
        expect(onRenderMenuHandler.mock.calls[0][2]).toEqual({ left: undefined, top: undefined, minWidth: undefined });
        // Re-render after componentDidMount
        expect(onRenderMenuHandler.mock.calls[1][2]).toEqual({ left: 0, top: 0, minWidth: 0 });
    });
