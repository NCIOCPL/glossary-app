    ...
    switch(payload.type) {
      case 'Other': {
        const { 
           ...
           name, title, metaTitle, language, audience, channel, contentGroup, publishedDate,
      ...
      case 'PageLoad': {
        const { 
           type, event, name, title, metaTitle, language,
           audience, channel, contentGroup, publishedDate,
           ...additionalDetails
        } = payload;
        window.NCIDataLayer.push({
           type,
