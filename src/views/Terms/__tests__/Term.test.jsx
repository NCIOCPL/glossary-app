...
describe('Term component rendered with English', () => {
		...
		return render(
			<MockAnalyticsProvider analyticsHandler={analyticsHandler}>
				<MemoryRouter initialEntries={['/']}>
					<TermWithLocation term={term} />
				</MemoryRouter>
			</MockAnalyticsProvider>
		);
	};

	it.each(testTerms)('Check term title exists as link, click on link and confirm correct location when term has %s', async (termType, termObject, expectedLocationObject) => {
		renderComponent(termObject);

		// Get term title link
		const termTitleLink = screen.getByRole('link');
		// Expect term title link text to match term name
		expect(termTitleLink).toHaveTextContent(termObject.termName);
	...
	it.each(testTerms)('Term description is displayed for term with %s', async (termType, termObject) => {
		renderComponent(termObject);

		expect(screen.getByTestId(testIds.TERM_ITEM_DESCRIPTION)).toBeInTheDocument();
	});

	it.each(testTerms)('Term result click analytics event', async (termType, termObject) => {
		renderComponent(termObject);
